...
pretrain_name=${pretrain_split}_res50_scheduling_sgd_lr${pretrain_lr}_temp${temp}_epoch${pretrain_epochs}_batch${pretrain_batch_size}_s${seed}
...
--cvt_state_dict --world_size ${GPU_NUM} --num_workers ${workers} --test_freq 2 --seed ${linear_eval_seed}"
...
cmd_few_shot="python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} --master_port ${port} \
  ...
  --checkpoint checkpoints_imagenet/${pretrain_name}/model_${pretrain_epochs}.pt \
  --cvt_state_dict --world_size ${GPU_NUM} --port ${port} --num_workers ${workers} --test_freq 10"
