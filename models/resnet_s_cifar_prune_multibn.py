...
from .resnet_prune_multibn import batch_norm_multiple, proj_head, proj_head_dual
from collections import OrderedDict


__all__ = ['resnet32_prune_dual_s']
...
class ResNet_s(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, use_norm=False, bn_names=["nonprune", "prune"]):
        ...
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2, bn_names=self.bn_names)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2, bn_names=self.bn_names)
        if use_norm:
...
def resnet32_prune_dual_s(num_classes=10, use_norm=False):
    return ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, use_norm=use_norm)


class BBN_ResNet_Cifar(nn.Module):
    def __init__(self, block, num_blocks, num_classes, bn_names=["nonprune", "prune"]):
        ...
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1, bn_names=self.bn_names)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2, bn_names=self.bn_names)
        self.layer3 = self._make_layer(block, 64, num_blocks[2] - 1, stride=2, bn_names=self.bn_names)
        self.cb_block = block(self.in_planes, self.in_planes, stride=1, bn_names=self.bn_names)
        self.rb_block = block(self.in_planes, self.in_planes, stride=1, bn_names=self.bn_names)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, add_flag=True, bn_names=None):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, bn_names=bn_names))
    ...
    def forward(self, x, **kwargs):
        ...
        out = torch.cat((out1[0], out2[0]), dim=1)
...
def resnet32_BBN_prune_dual_s(num_classes=10, use_norm=False):
    assert not use_norm
