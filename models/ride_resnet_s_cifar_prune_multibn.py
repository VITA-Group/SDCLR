...
from models.resnet_prune import prune_conv2d
from models.resnet_prune_multibn import batch_norm_multiple, proj_head, proj_head_dual
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A', bn_names=None):
        super(BasicBlock, self).__init__()
        self.conv1 = prune_conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = batch_norm_multiple(nn.BatchNorm2d, planes, bn_names=bn_names)
        self.conv2 = prune_conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = batch_norm_multiple(nn.BatchNorm2d, planes, bn_names=bn_names)
...
class ResNet_s(nn.Module):
    def __init__(self, block, num_blocks, num_experts, num_classes=10, reduce_dimension=True, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, use_experts=None, bn_names=["nonprune", "prune"], s=30):
        ...
        self.bn1 = batch_norm_multiple(nn.BatchNorm2d, self.in_planes, bn_names=bn_names)
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1, bn_names=self.bn_names)
        ...
        self.layer2s = nn.ModuleList([self._make_layer(block, layer2_output_dim, num_blocks[1], stride=2, bn_names=self.bn_names) for _ in range(num_experts)])
        self.in_planes = self.next_in_planes
        self.layer3s = nn.ModuleList([self._make_layer(block, layer3_output_dim, num_blocks[2], stride=2, bn_names=self.bn_names) for _ in range(num_experts)])
        self.in_planes = self.next_in_planes
    ...
    def _make_layer(self, block, planes, num_blocks, stride, bn_names):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        self.next_in_planes = self.in_planes
        for stride in strides:
            layers.append(block(self.next_in_planes, planes, stride, bn_names=bn_names))
    ...
    def _separate_part(self, x, ind):
        ...
        self.feat_before_GAP.append(out[0])
    ...
    def forward(self, x):
        ...
        if self.fc is not None:
            outs = torch.cat(self.feat, dim=1)
            final_out = self.fc(outs, bn_name=bn_name)
            return final_out
...
def resnet32(num_classes=100, num_experts=4, use_norm=True, bn_names=["nonprune", "prune"]):
    return ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, use_norm=use_norm, bn_names=bn_names, num_experts=num_experts)
